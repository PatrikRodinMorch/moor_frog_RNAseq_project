...
"
concat_matrix_dexseq_liver.sh
for f in $FILES
do
    echo ${f}
    echo /crex/proj/snic2019-30-54/reference_guided/liver/rsem/rsem_${f}/rsem_${f}.genes.results >> files_genes.temp
    echo /crex/proj/snic2019-30-54/reference_guided/liver/rsem/rsem_${f}/rsem_${f}.isoforms.results >> files_isoforms.temp 

done

cat files_genes.temp | xargs rsem-generate-data-matrix > expression_matrix/rsem.counts.genes.matrix
cat expression_matrix/rsem.counts.genes.matrix | tail -n+2 > expression_matrix/rsem.counts.genes.matrix_no_header
cp NewHeader.txt expression_matrix/rsem.counts.genes.final.matrix
cat expression_matrix/rsem.counts.genes.matrix_no_header >> expression_matrix/rsem.counts.genes.final.matrix



cat files_isoforms.temp | xargs rsem-generate-data-matrix > expression_matrix/rsem.counts.isoforms.matrix
cat expression_matrix/rsem.counts.isoforms.matrix | tail -n+2 > expression_matrix/rsem.counts.isoforms.matrix_no_header
cp NewHeader.txt expression_matrix/rsem.counts.isoforms.final.matrix
cat expression_matrix/rsem.counts.isoforms.matrix_no_header >> expression_matrix/rsem.counts.isoforms.final.matrix
